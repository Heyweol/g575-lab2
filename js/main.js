...
var StateData;
var stateBoundaryJson;

/*
access properties through leaflet event: e.target.feature.properties.{State_Code/State_Name/FID}
 */
...
function createMap() {
    ...
    promises.push(d3.json("data/States_shapefile.geojson"))
    Promise.all(promises).then(callback);

    function callback(data) {
        csvData = data[0];
        csvData2 = data[1];
        stateBoundaryJson = data[2]
        ...
        stateChoroplethLayer = L.geoJson(stateBoundaryJson,{
            style:defaultChoroplethStyle,
...
function defaultChoroplethStyle(feature) {
    return {
        fillColor: getColorByState(feature.properties.State_Code),
        weight: 1,
        opacity: 1,
        // color: 'white',
        dashArray: '3',
...
function getColorByState(StateCode) {
    var colorClasses = [
        "#ffffcc",
        "#a1dab4",
        "#41b6c4",
...
function highlightChoroplethStyle(e) {
    layer = e.target;
    ...
    highlight(
        csvData.filter(
            (props)=>props.state===layer.feature.properties.State_Code)[0])
}

function resetChoroplethStyle(e) {
    layer = e.target;
    dehighlight(csvData.filter((props)=>props.state===layer.feature.properties.State_Code)[0])

    // console.log(layer)
    stateChoroplethLayer.resetStyle(layer);
    // layer.setStyle({
    //     weight: 1,
    //     color: 'none',
    //     dashArray: '',
...
function onEachFeatureChoropleth(feature, layer) {
    layer.on({
        mouseover: highlightChoroplethStyle,
        mouseout: resetChoroplethStyle,
        click: zoomToFeature
    });
}
...
function calcPropRadius(attValue) {
    ...
    var radius;
    if (attValue > 0) {
        radius = 1.0083 * Math.pow(attValue / 1, 0.2715) * minRadius
    } else {
        radius = 0
    }
    // console.log(radius)
...
function createPropSymbols(data, attributes) {
    ...
    let propSymbols = L.geoJson(data, {
        pointToLayer: function (feature, latlng) {
            return pointToLayer(feature, latlng, attributes);
        }
    }).addTo(map);
    propSymbols.eachLayer(function (layer) {
        if (layer.feature.properties[expressed] === 0) {
            layer.remove()
        }
    })
};
...
function changeAttribute(attribute, csvData) {
    ...
    stateChoroplethLayer.setStyle(defaultChoroplethStyle)
...
function highlight(props) {
    ...
    // console.log(props.state) // state code
...
function secondDropdown() {
    ...
    var secondDrop = d3.select("body")
        ...
        .on("change", function () {
            ...
            console.log("changed second drop")
        });
...
function changeAttribute2(attribute2) {
    ...
    csvData2.forEach(function (player) {
        if (player.STATE === expressed2) {
            ...
            totalHeight += parseFloat(player.HEIGHT);
            totalWeight += parseFloat(player.WEIGHT);
            totalRank += parseFloat(player.RANK);
            totalStars += parseFloat(player.STARS);
            playerCount++;
        }
    });
