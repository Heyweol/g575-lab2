...
function onEachFeatureChoro(feature,layer){
    layer.on({
            mouseover: highlightState,
            mouseout: dehighlightState,
        }
...
function getData() {
    ...
    d3.json("data/states.topojson").then(function (topoJsonData) {
        statePath = topoJsonData
        var geojsonData = topojson.feature(topoJsonData, topoJsonData.objects.States_shapefile);
        var statesLayer = L.geoJson(geojsonData, {
            style: styleFunction,
            onEachFeature: onEachFeatureChoro
        }).addTo(map);

        // Calculate mean metrics for each state and bind data to features
        calculateMeanMetrics(geojsonData);
    });
...
function styleFunction(feature) {
    var meanValue = feature.properties.meanMetric;
    var fillColor = getColorByValue(meanValue);
...
function calculateMeanMetrics(geojsonData) {
    geojsonData.features.forEach(function (feature) {
        var stateCode = feature.properties.stateCode;
        if(csvData2===undefined){return 0}
        var filteredPlayers = csvData2.filter(function (player) {
            return player.STATE === stateCode;
        });

        var meanMetric = d3.mean(filteredPlayers, function (player) {
            return parseFloat(player[expressed]);
        });

        feature.properties.meanMetric = meanMetric;
    });
}
